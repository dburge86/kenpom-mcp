...
def parse_hca(soup: BeautifulSoup) -> list[dict]:
    ...
    if HAS_PANDAS:
        ...
        columns = [
            "Rank", "Team", "Conference", "HCA", "HCA_Rank",
            "PF", "PF_Rank", "Home_Record", "Pts_Diff"
        ]
...
def parse_game_attrs(soup: BeautifulSoup) -> list[dict]:
    ...
    if HAS_PANDAS:
        ...
        columns = [
            "Rank", "Date", "Game", "Box", "Location", "Conf_Matchup", "Value"
        ]
        df.columns = columns[:len(df.columns)]
...
def parse_program_ratings(soup: BeautifulSoup) -> list[dict]:
    ...
    if HAS_PANDAS:
        ...
        columns = [
            ...
            "Best_Rank", "Best_Season", "Worst_Rank", "Worst_Season",
            "Median_Rank", "Top10", "Top25", "Top50",
            "NCAA_Champs", "NCAA_F4", "NCAA_S16", "NCAA_R1", "Change"
        ]
        df.columns = columns[:len(df.columns)]
        df = df[df["Team"] != "Team"]
